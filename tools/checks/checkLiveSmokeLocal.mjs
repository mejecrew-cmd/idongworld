...
function start(name, args) {
  console.log(`[live-smoke:local] start ${name}: pnpm ${args.join(' ')}`)
  ...
  child.on('exit', (code, signal) => {
    if (code !== null && code !== 0 && !isCleaningUp) {
      console.warn(`[live-smoke:local] ${name} exited with code ${code}`)
    }
    if (signal && !isCleaningUp) {
      console.warn(`[live-smoke:local] ${name} exited by ${signal}`)
    }
  })
  return child
}
...
function run(args) {
  return new Promise((resolve, reject) => {
    console.log(`[live-smoke:local] run: pnpm ${args.join(' ')}`)
    const command = pnpmCommand(args)
    const child = spawn(command.command, command.args, {
      stdio: 'inherit',
      env: localEnv,
...
async function waitFor(name, probe, timeoutMs = 30_000) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    try {
      await probe()
      console.log(`[live-smoke:local] ready ${name}`)
      return
    } catch (error) {
      lastError = error
...
async function cleanup() {
  isCleaningUp = true
  console.log('[live-smoke:local] cleanup')
...
async function main() {
  ...
  await run(['check:module-actions-smoke'])
  await run(['check:e2e:smoke'])
  await run(['check:state-route-runtime'])
  console.log('[live-smoke:local] ok')
}

main()
  .catch((error) => {
    console.error(`[live-smoke:local] failed: ${error.message}`)
    process.exitCode = 1
  })
  .finally(async () => {
    await cleanup()

...
## 1.6. 동적 스키마 전환 기준 2026-06-29

동적 유저 데이터는 xlsx 정책에 맞춰 normalized row 중심으로 분리했다.

신규 backend 작업은 아래 권위 저장소를 우선한다.

- 인증 provider 연결: `providerAccounts`
- 설정, 약관, 마케팅, 푸시, 사운드: `userSettings`
- coin/diamond: `userCurrencyBalances`, 변경 이력은 `userCurrencyLedger`
- 주사위: `diceResources`
- 전역 인벤토리: `userInventoryItems`
- 숙소 청소/이름: `sooksoStates`
- 숙소 방 배정: `roomSlots`
- 숙소 방 가구 배치: `roomFurniturePlacements`
- 보유 Aidong: `mydongList`
- Aidong 도감템 수량: `mydongPediaInventory`
- 코스메틱 보유: `userCosmeticInventory`
- 코스메틱 장착: `mydongCosmeticLoadouts`
- persona part 상태: `mydongPersonaPartStates`

`users`, `hostStates`, `myAidongStates`, `lodgeStates`의 기존 필드는 API 호환과 migration fallback을 위한 mirror/projection으로 남아 있다. 새 기능은 이 필드를 직접 권위로 삼지 않는다.

운영 migration 절차는 `.cloud/105_dynamic_schema_migration_runbook_2026-06-29.md`를 따른다.
...
- `providerAccounts`: provider별 인증 연결. 한 유저가 여러 provider를 가질 수 있다.
- `userSettings`: 약관, 마케팅, 푸시, 시간대, 사운드 설정.
- `hostStates`: host 이름과 기존 API 호환 projection. 광역 재화/주사위/inventory의 단독 권위가 아니다.
- `userCurrencyBalances`: coin/diamond 잔액.
- `userCurrencyLedger`: 재화 변경 이력.
- `diceResources`: 주사위 수량, 충전 메타, 일일 roll 메타.
- `userInventoryItems`: 전역 인벤토리 item row.
- `myAidongStates`: Aidong needs, 호감도, 케어 로그 등 호환 상태.
- `mydongList`: 유저가 보유한 개별 Aidong row. `mydongUid`와 `aidongId`를 구분한다.
- `mydongPediaInventory`: Aidong별 도감템 수량.
...
- 아이동별 25개 도감 아이템 수량은 `codexStates`가 아니라 `mydongPediaInventory`가 소유한다.
- `myAidongStates.aidongCodexItems`는 호환 mirror로만 본다.
- 아이동 업그레이드 상태는 현재 `myAidongStates.aidongUpgradeState` 후보 필드가 소유한다.
...
- 일반 재료 비용이 필요한 업그레이드는 `userInventoryItems` 차감과 같은 트랜잭션 단위로 처리한다.
...
- Google/Twitter/password provider metadata는 `providerAccounts`에 저장한다.
- `users.authProvider`, `users.providerUid`, `users.email`, `users.displayName`, `users.photoURL`은 호환 mirror로 유지할 수 있다.

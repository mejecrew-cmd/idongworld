...
| `voyage-route` | 항해 경로와 보드 규칙. 현재 항해 진행 상태는 브라우저 세션 전용 | route-neighbor |
...
## 4. 저장소 소유권을 정한다
### 4.0 항해형 모듈의 세션/DB 분리

항해형 모듈을 만들 때 가장 먼저 구분해야 하는 것은 “현재 플레이 중인 세션 상태”와 “계정에 영구 반영되는 결과”다.
...
- 현재 항해 route id.
- 현재 보드 칸.
- 이번 탭에서만 유효한 주사위 이동 결과.
- 이번 탭에서만 유효한 landing 후보.
- 항해 창을 닫으면 사라져도 되는 임시 선택 상태.

DB에 저장하는 값:

- 주사위 소모와 지급.
- host inventory 증가/감소.
- Aidong 영입.
- 마이섬 slot 편입.
- 도감템, upgrade 재료, 숙소 성장값처럼 계정에 남아야 하는 보상.
- 감사 로그가 필요한 cross-module 자원 이동 결과.
...
### 9.5 resources/table 정적 CSV와 module-local CSV 구분

정적 운영 테이블과 module package 내부 CSV는 용도가 다르다.

`resources/table/*.csv`:

- 기획/운영에서 제공하는 전역 기준 데이터다.
- Admin DB 관리 화면의 validation, dry-run, commit, activate 절차를 통해 MongoDB 정적 collection과 JSON bundle로 반영한다.
- 예: Aidong master, item catalog, currency master, island zone, board slot, sookso rule, string, dialogue, story, Aidong island 기준 데이터.
- 런타임에서는 `/api/static/*` 또는 backend static table repository를 통해 activate된 batch만 읽는다.

`packages/modules/{moduleId}/*.csv`:

- 특정 module package 안에서만 쓰는 local balance, item, decor, customs, i18n 후보 데이터다.
- module parser, manifest, audit, backend service 검증과 함께 관리한다.
- module-local CSV를 운영 정적 table import에 자동으로 태우지 않는다.

금지:

- `resources/table` import로 유저별 진행 상태를 만들거나 덮어쓰지 않는다.
- `G-USR-01`, `M01-USR-01`, `M05-ECO-01`, `M17-ECO-01`, `M19-USR-01`, `M21-USR-01`, `M22-USR-*`, `M23-ITM-01` 같은 동적 유저 데이터 테이블을 module seed처럼 취급하지 않는다.
- 유저별 상태가 필요하면 dedicated collection, repository, service, API를 만든다.
...
- [ ] `resources/table` 정적 CSV인지 module-local CSV인지 구분했다.
- [ ] 유저별 동적 데이터가 정적 table import로 들어가지 않는다.
...
- **2026-06-29**: `resources/table` 정적 CSV와 module-local CSV의 차이를 추가했다. 정적 import는 운영 기준 데이터용이며, 동적 유저 데이터는 전용 repository/API/migration으로 처리한다.

- **2026-06-13**: 모듈 제작자가 신규 모듈을 만들 때 따라갈 수 있도록 제작 순서, 파일 위치, backend/frontend 연결, 보상 소유권, customs 판단, 검증 기준, 완료 체크리스트를 한 문서로 정리했다.

- **2026-06-13**: 항해형 모듈 제작 시 현재 항해 상태는 브라우저 탭/창별 세션에만 두고, DB에는 주사위 소모와 보상 같은 영속 결과만 기록한다는 규칙을 추가했다.

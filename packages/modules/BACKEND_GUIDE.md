...
이 문서는 `packages/modules` 아래에서 모듈을 개발할 때 backend 저장소와 API를 어떻게 설계해야 하는지 정리한다. 신규 모듈의 전체 제작 순서는 `.cloud/02_module_creator_guide_2026-06-13.md`를 먼저 보고, 이 문서는 backend 세부 기준으로 사용한다.
...
- Phase 1 core loop와 상태 소유권 1차 메모는 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 항해 아이동 만남과 메인 가변 구역 추가의 1차 계약은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 통 인벤토리와 아이동별 25 도감 아이템 소유권은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 세관 de-scope 기준은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 숙소 중심 육성 허브 기준은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 구역 배치 자동 생산 기준은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 아이동별 미니게임 스킨 기준은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 도감 아이템과 숙소 업그레이드 경제 기준은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 항해 보드 단순화 결정 기준은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
- 항해 중/정박 중 여부와 현재 칸은 DB가 아니라 브라우저 탭/창별 세션 상태다.
- 배 적재량과 입항/비우기 사이클 기준은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
...
- 자세한 충돌 분류는 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
...
- account/session: account API와 account repository. 단, 항해 플레이 세션은 account DB state가 아니라 브라우저 탭/창별 session state다.
...
상세 기준은 `.cloud/01_project_history_current_2026-06-13.md`를 따른다.
...
- 세관 UI 노출이나 이탈 gate 재활성화는 `.cloud/01_project_history_current_2026-06-13.md`의 feature flag와 재개 조건을 먼저 확인한다.
...
2026-06-13 기준으로 항해 진행 상태는 DB에 저장하지 않는다.

핵심 원칙:

- DB는 현재 배가 출항 중인지 항구에 정박 중인지 모른다.
- DB는 현재 route id, 현재 칸, 세션 내 landing 후보를 권위 상태로 저장하지 않는다.
- 항해 진행 상태는 브라우저 탭/창별 session state다.
- 한 계정이 여러 항해 창을 열면 각 창은 서로 독립적인 항해 세션을 가진다.
- 항해 창을 닫으면 해당 세션의 항해 정보는 사라진다. 게임적으로는 자동 마이섬 복귀로 본다.
- 항해 중인 탭이 있어도 다른 탭이나 직접 URL로 항구에 들어가면 항구는 배가 정박한 화면을 보여준다.
- 항구 화면에서 “배가 출항 중입니다” 같은 DB 기반 문구를 띄우지 않는다.
- 항구에서 항해 시작을 누르면 그 탭의 항해 세션을 처음부터 새로 만든다.

`route-neighbor`가 맡을 수 있는 것:

- 항해 route catalog.
- board size, roll rule, landing generator 같은 규칙.
- roll 결과 후보 반환.
- landing/encounter 보상 확정 action.
- 주사위 소모, 보상 지급, Aidong 영입 같은 영속 결과 반영.

`route-neighbor`가 맡으면 안 되는 것:

- `currentRoute`를 user의 현재 항해 상태로 영속 저장하는 것.
- `boardPosition`을 user의 현재 칸으로 영속 저장하는 것.
- 한 탭의 출항 상태를 다른 탭의 항구 접근에 강제하는 것.

`ship`이 맡을 수 있는 것:

- `shipTypeId`: 현재 선택된 배 종류.
- `cabinAssignments`, `deckAssignments`: 영속 배치/호환 상태. 단, 항해 세션 lock으로 쓰지 않는다.
- `cabinFurniture`, `cabins`: 선실 꾸미기 구매/배치 상태.
- `shipInventory`: 새 통 인벤토리 설계 전까지 유지하는 선박 inventory 호환 필드.
- `cargoCapacity`: `shipStates`에 저장하지 않고 `packages/modules/ship/balance.csv`의 현재 `shipTypeId` config에서 계산한다.

`ship`이 맡으면 안 되는 것:

- 현재 배가 출항 중이라는 user-level flag.
- 현재 항해 세션의 위치.
- 여러 항해 창을 막는 mutex.
...
새 항해 기능을 붙일 때는 먼저 “이 값이 창을 닫으면 사라져도 되는가?”를 묻는다. 사라져도 된다면 frontend session state에 둔다. 계정에 남아야 한다면 해당 권위 module action API로 DB에 기록한다.
## Lodge 육성 허브 상태
...
- 항해 중인 Aidong은 특정 브라우저 세션 안에서만 항해 중으로 취급한다. DB에 저장된 전역 출항 flag로 숙소 배치를 막지 않는다.
...
- Aidong이 영입되어 있어야 한다.
- 특정 브라우저 세션에서 항해 중인 Aidong은 그 세션 UI 안에서만 배치 제한을 둘 수 있다. DB 전역 출항 flag로 배치를 막지 않는다.
...
- `packages/modules/my-aidong/minigame-skins.csv`를 최소 skin catalog로 사용한다. 별도 minigame catalog 분리는 후속 후보로 둔다.
...
- **2026-06-08**: Backend/API 영향도 조사 문서를 연결했다. Phase 1에서 유지할 route, 숨길 route, 변경할 route와 신규 API 후보는 `.cloud/01_project_history_current_2026-06-13.md`를 기준으로 확인한다.
...
- **2026-06-08**: Phase 1 core loop와 상태 소유권 1차 메모 기준으로 `.cloud/01_project_history_current_2026-06-13.md`를 연결했다.
- **2026-06-08**: 항해 아이동 만남과 메인 가변 구역 추가 설계 기준으로 `.cloud/01_project_history_current_2026-06-13.md`를 연결했다.
- **2026-06-08**: 통 인벤토리와 아이동 귀속 도감 인벤토리 설계 기준으로 `.cloud/01_project_history_current_2026-06-13.md`를 연결했다.
...
- **2026-05-29**: Route/Ship 항해 상태의 임시 경계를 추가했다. 이 기준은 2026-06-13 항해 세션 권위 분리 결정으로 대체되었으며, route-neighbor/ship은 현재 출항 여부와 현재 칸을 DB에 저장하지 않는다.

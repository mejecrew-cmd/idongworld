/**
 * 📁 zone-oasis/src/ZoneScreen.tsx — 오아시스 zone 진입 화면 (자율 등록 라우트)
 * ───────────────────────────────────────────────
 * 📌 역할: /island/oasis 진입 시 zone 화면 표시.
...
export const ZoneScreen = () => {
  ...
  const { t } = useTranslation('zone-oasis')
  return (
    <Box sx={{ minHeight: '100vh', p: 2 }}>
      <Stack direction="row" alignItems="center" sx={{ mb: 2 }}>
        <Button size="small" onClick={() => navigate('/island/full-map')}>{t('back_to_full_map', '← 풀맵')}</Button>
        <Typography variant="h2" sx={{ fontSize: 18, ml: 2 }}>{t('title', '🏖️ 오아시스')}</Typography>
      </Stack>
